...
def fun():
	...
	if nome != '':
		...
		for w in sht.worksheets():
			...
			index_inprova = lower_title.find("- probationary member")
			if (index_inprova >= 0):
		...
		if double_inprova > 1 and (in_prova):
			st.warning('Multiple probationary members have been found with this name/surname; please try to be more specific.')
			return
		else:
			if in_prova: 
				if double_inprova == 0:
					st.error('No resource matching the search criteria has been found')
					return
		...
		# --- adding elements to google sheet ---
			...
			st.success(f"{current_work.title}'s Hour Count has been updated")
	else:
		st.warning('First name and/or Last name are required')
	return

# --- Interfaccia ----
nome = st.text_input('First name and/or Last name. Check the box if you are a probationary member')
in_prova = st.checkbox('Probationary Member')
...
for a in att:
	if a == "External Project":
		...
		progetti_in_corso = ["Farmacie del Corso", "EveryBotics", "COSOP", "Olio SB"]
